...
func TestShouldPublishRouterCA(t *testing.T) {
	...
	testCases := []struct {
		...
		input		[]*string
		output		bool
	}{{"no ingresses", []*string{}, false}, {"all using no default certificates", []*string{&empty, &empty, &empty}, false}, {"all using generated default certificates", []*string{nil, nil, nil}, true}, {"all using custom default certificates", []*string{&x, &x, &y}, false}, {"mix of custom default certificate and no default certificate", []*string{&x, &x, &empty}, false}, {"mix of custom default certificate and generated default certificate", []*string{&x, &x, nil}, true}, {"mix of no default certificate and generated default certificate", []*string{&empty, &empty, nil}, true}}
	for _, tc := range testCases {
		...
		for i := range tc.input {
			...
			ingress := operatorv1.IngressController{ObjectMeta: metav1.ObjectMeta{Name: fmt.Sprintf("router-%d", i)}, Spec: operatorv1.IngressControllerSpec{Replicas: &replicas, DefaultCertificate: cert}}
