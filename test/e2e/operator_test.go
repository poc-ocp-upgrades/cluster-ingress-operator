...
func TestIngressControllerUpdate(t *testing.T) {
	...
	if deployment.Spec.Template.Spec.Volumes[0].Secret.SecretName != expectedSecretName {
		t.Fatalf("expected router deployment certificate secret to be %s, got %s", expectedSecretName, deployment.Spec.Template.Spec.Volumes[0].Secret.SecretName)
	}
...
-----END RSA PRIVATE KEY-----
`
	secret := &corev1.Secret{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: "openshift-ingress"}, Data: map[string][]byte{"tls.crt": []byte(defaultCert), "tls.key": []byte(defaultKey)}, Type: corev1.SecretTypeTLS}
...
func TestRouterCACertificate(t *testing.T) {
	...
	conn, err := tls.Dial("tcp", address, &tls.Config{RootCAs: certPool, ServerName: serverName})
	if err != nil {
